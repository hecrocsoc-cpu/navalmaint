...
function tareaPendiente(tarea, logs) {
  ...
  const diasDesdeUltimo =
    (Date.now() - new Date(ultimoLog.fecha)) / (1000 * 60 * 60 * 24);
...
export default function Dashboard() {
  ...
  const cargar = async () => {
    try {
      ...
      const totalEquipos = equiposPorbuque.reduce(
        (acc, eq) => acc + eq.length,
        0,
      );
      ...
      const logsFiltrados =
        usuario?.role === "MECANICO"
          ? logsData.filter(
              (l) => l.task?.equipment?.vesselId === usuario.vesselId,
            )
          : logsData;
      ...
      const tareasFiltradas = tareas.filter((t) => {
        ...
        const esFrecuenciaRelevante = ["DIARIA", "SEMANAL"].includes(
          t.frecuencia,
        );
        return esDelbuque && esFrecuenciaRelevante;
      });

      const pendientes = tareasFiltradas.filter((t) =>
        tareaPendiente(t, logsData),
      );
  ...
  return (
    <div className="page-container">
      ...
      <div className="cards-grid" style={{ marginBottom: 32 }}>
        ...
        >
          <h3 style={{ color: "#4a9eff", fontSize: "2rem", marginBottom: 4 }}>
            {loading ? "—" : stats.buques}
          </h3>
          <p>Buques</p>
        </div>
        <div
        ...
        <div
          ...
          style={{
            cursor: "pointer",
            borderColor: stats.alertas > 0 ? "#ef4444" : undefined,
          }}
        >
          <h3
            style={{
              color: stats.alertas > 0 ? "#ef4444" : "#4a9eff",
              fontSize: "2rem",
              marginBottom: 4,
            }}
          >
                ...
                <>
                  ...
                  {formulario === t.id && (
                    <tr key={`form-${t.id}`}>
                      <td
                        colSpan={5}
                        style={{ background: "#0f1e36", padding: "16px 20px" }}
                      >
                        <form
